...
public class NLivetestController implements Initializable {
	...
	double stepsizepercentage = 0.2, maxpressureinstepsize = 1, mindelay = 200, maxdelay = 10000, minavg = 2,
			maxavg = 12;
	...
	// set all shortcut
	void addShortCut() {
		KeyCombination backevent = new KeyCodeCombination(KeyCode.E, KeyCombination.CONTROL_ANY);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		System.out.println("All Live data--->Sample Plate-->" + Myapp.splate + "--End Press--->" + Myapp.endpress
				+ "--thickness-->" + Myapp.thikness + "---Accu Poro-->" + Myapp.accstep + "---Accu statbility--->"
				+ Myapp.accstability);
		...
		if (ss.equals("1")) {
			...
			System.out.println("---Test Typr--->" + Myapp.testtype + "--Dk interval--->" + Myapp.dkdatainterval
					+ "--Dk hold time--->" + Myapp.dkholdtime + "--Dk pressure--->" + Myapp.dkpressure);
...
//		/setTemp();
	...
	void createCsvTable() {
		...
		//	dryplist.remove(3);
			...
			/*
			 * double frazierflow = c.getFlowPointOn(dryplist, dryflist, 50000,1, 0.0182);
			 * 
			...
			double gurleyflow = c.getFlowPointOn(dryplist, dryflist, 50000, 1, 0.178);
			String gurley = getDarcyGurley(gurleyflow);

			/*
			 * double frazierflow = c.getFlowPointOn(dryplist, dryflist, 500, 5, 0.36019);
			 * 
			...
			cs.newLine("flow05inch", "" + c.getFlowPointOn(dryplist, dryflist, 50000, 1, 0.0182));
			cs.newLine("flow10inch", "" + c.getFlowPointOn(dryplist, dryflist, 50000, 1, 0.3));
	...
	List<Integer> getStepSize() {
		...
		System.out.println("Step Size : " + min);
	...
	void setPermiabilityPoints(double pr, double fl) {
		...
		if (curpre < conditionpressure && curflow < conditionflow && isSkiptest == false) {
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					//	series2.getData().add(new XYChart.Data(curpre, curflow));
						series2.getData().add(
								new XYChart.Data(DataStore.ConvertPressure(curpre), DataStore.ConvertFlow(curflow)));
						dryplist.add("" + curpre);
						dryflist.add("" + (curflow / 60));
						ind++;
					} else {
					...
					flowserireswet.getData().add(
							new XYChart.Data(getTime(), DataStore.ConvertFlow(curflow)));
					pressureserireswet.getData().add(
							new XYChart.Data(getTime(), DataStore.ConvertPressure(curpre)));
				//	flowserires.getData().add(new XYChart.Data(getTime(), curflow));
				//	pressureserires.getData().add(new XYChart.Data(getTime(), curpre));
