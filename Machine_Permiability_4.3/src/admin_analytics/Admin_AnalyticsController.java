...
void setOrderwithdate()
{
	...
	if (ls.size()>0) {
		...
		dropyear1.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
			...
			public void changed(
				...
				setBarchartorder(data.get(Integer.parseInt(newValue)), newValue);
...
void setUsers()
{
	...
	if (ls.size()>0) {
		...
		dropyear.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
			...
			public void changed(
				...
				setBarchart(data.get(Integer.parseInt(newValue)), newValue);
