 ...
 class ChartsCreationTask extends Task<Void> {
   ...
   @Override protected Void call() throws Exception {
     ...
     while (i > 0) {
       ...
       if(i==1)
       {
    	   Pane p=c.drawLinechart(pagination1.getPrefWidth(),pagination1.getPrefHeight(),"Flow vs Pressure", "Pressure (psi)", "Flow (cm3/s)",list_d,false,11,12,"(3) Incremental Filter-Flow % vs Diameter");
    	   charts.put(p);
    	   listofchart.add(p);
	...
	private void createChartImagePagination(final SaveChartsTask saveChartsTask) {
	   ...
	   pagination1.setPageFactory(new Callback<Integer, Node>() {
	     @Override public Node call(final Integer chartNumber) {
	    	 ...
	    	 if(chartNumber==0)
	    	 {
	    		 grlable.setText("Flow vs Pressure");
	    	 }
