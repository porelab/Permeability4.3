...
public class OnlinefilepopupController implements Initializable {
	...
	void saveFile() {
		...
		if (fileno >= listofdownloadfile.size()) {
			...
			DocumentReference reff=FirebaseConnect.db.collection("users").document(Myapp.uid).collection("files").document("permiability").collection("files").document(name);
