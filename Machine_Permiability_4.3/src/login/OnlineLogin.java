...
public class OnlineLogin implements Initializable {
	...
	void onlineLoginMethod() throws UnknownHostException {
		try {
			...
			Query query = FirebaseConnect.db.collection("users")
					.whereEqualTo("email", txtemail.getText())
					.whereEqualTo("pass", txtpass.getText());
			;
			...
			if (querySnapshot.get().getDocuments().size() > 0) {
				...
				insertTable(data.get("userid") + "", data.get("email") + "",
						data.get("pass") + "", data.get("name") + "",
						data.get("companyname") + "");
				// Toast.makeText(Main.mainstage, "Login Successfully", 2000,
				// 100, 100);
				System.out.println("login successfully");

			} else {
				isInternet = true;
				Query query1 = FirebaseConnect.db.collection("users")
						.whereEqualTo("userid", txtemail.getText())
						.whereEqualTo("pass", txtpass.getText());
				;
				ApiFuture<QuerySnapshot> querySnapshot1 = query1.get();
				if (querySnapshot1.get().getDocuments().size() > 0) {
					DocumentSnapshot dd1 = querySnapshot1.get().getDocuments()
							.get(0);
					Map<String, Object> data = (Map<String, Object>) dd1
							.getData();
					insertTable(data.get("userid") + "",
							data.get("email") + "", data.get("pass") + "",
							data.get("name") + "", data.get("companyname") + "");
					// Toast.makeText(Main.mainstage, "Login Successfully",
					// 2000, 100, 100);
					System.out.println("login successfully");
	...
	void Offlinelogin() {
		...
		boolean isId = false;
		if (d.isExist("select * from userinfo where email='"
				+ txtemail.getText() + "' and pass='" + txtpass.getText() + "'")) {
			String updatequry = "update userinfo set login='yes' where email='"
					+ txtemail.getText() + "'";
			if (d.Insert(updatequry)) {
				Database d1 = new Database();
				List<List<String>> alldata = d1
						.getData("select * from userinfo where email='"
								+ txtemail.getText() + "' and pass='"
								+ txtpass.getText() + "' ");
				...
				.isExist("select * from userinfo where uid='"
						+ txtemail.getText() + "' and pass='"
						+ txtpass.getText() + "'")) {
			String updatequry = "update userinfo set login='yes' where uid='"
					+ txtemail.getText() + "'";
			if (d.Insert(updatequry)) {
				Database d1 = new Database();
				List<List<String>> alldata = d1
						.getData("select * from userinfo where uid='"
								+ txtemail.getText() + "' ");
	...
	void insertTable(String uid, String email, String pass, String name,
			String indname) {
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				if (!d.isExist("select * from userinfo where uid='" + uid + "'")) {
					...
					String upquery = "update userinfo set email='" + email
							+ "',pass='" + pass + "',name='" + name
							+ "',login='yes',indname='" + indname
							+ "' where uid='" + uid + "'";
					if (d.Insert(upquery)) {
			...
			public void handle(ActionEvent arg0) {
				...
				if (!txtemail.getText().trim().equals("")
					...
					Toast.makeText(Main.mainstage,
							"Fill up the required details.", 3000, 500, 500);
				}
