...
public class Sync {
	...
	void uploadFile(List<String> saname)
	{
		...
		else
		{
			...
			DocumentReference reff1=FirebaseConnect.db.collection("users").document(Myapp.uid).collection("files").document("permiability");
			
		
		DocumentReference reff=FirebaseConnect.db.collection("users").document(Myapp.uid).collection("files").document("permiability").collection("files").document(saname.get(ind));
